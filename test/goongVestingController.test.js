...
describe("GoongVestingController", function () {
  ...
  beforeEach(async () => {
    ...
    const VestingControllerContract = await ethers.getContractFactory("GoongVestingController")
    vestingControllerContract = await VestingControllerContract.deploy(goong.address, vestingContract.address)
  ...
  describe("claimOwnership", async function () {
    beforeEach(async () => {
      await vestingContract.transferOwnership(vestingControllerContract.address)
        .then(tx => tx.wait())
    })
    ...
    it('should not transfer ownership if the caller is not owner', async function() {
      ...
      await expect(vestingControllerContract.connect(hacker).claimOwnership())
  ...
  describe('batchAddTokenVesting', async function() {
    beforeEach(async () => {
      await vestingContract.transferOwnership(vestingControllerContract.address)
        .then(tx => tx.wait())
    })
